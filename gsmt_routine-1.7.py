...
class Box:   
    ...
    def in_box(self, state):
        ...
        """
        xmin, xmax, ymin, ymax = self.grenzen
        x, y = state[:2]
...
def step(teilchen_sam, box, dt):
    ...
    for t, s_neu in zip(teilchen_sam, neue_states):
        t.state_neu(s_neu)
#
#
def main():
    ...
    with open("Werte_Simulation.txt", "w") as f:
        ...
        for durchlaufe in range(durchlauf):                     
            for array in teilchen:
                ...
                En= E(np.array([t.s for t in teilchen]))                     
...
for i in range(len(teilchen)):
    plt.plot(bes_x[i], bes_y[i], label= f"{i}. Teilchen", color= farben[i])
plt.xlabel("x")
plt.ylabel("x")
plt.legend()
